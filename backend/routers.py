...
from .jwt_Authentication import login_for_access_token, create_access_token, get_password_hash, ACCESS_TOKEN_EXPIRES_MINUTES
...
@router.get("/get_list", response_model=list[ResumeInDB])
def get_list_of_resumes(db: Annotated[Session, Depends(get_db)], current_user:Annotated[User, Depends(get_current_user)]):
...
def get_one_resume(resume_id: int, db: Annotated[Session, Depends(get_db)], current_user:Annotated[User, Depends(get_current_user)]):
    user_id = current_user.id
...
def create_resume(new_resume: ResumeCreate, db: Annotated[Session, Depends(get_db)], current_user:Annotated[User, Depends(get_current_user)]):
    user_id = current_user.id
...
def update_old_resume(
    ...
    query = update(Resume).where(Resume.id == resume_id, Resume.user_id == user_id).values(update_data)
...
def delete_all_resumes(db: Annotated[Session, Depends(get_db)], current_user:Annotated[User, Depends(get_current_user)]):
    user_id = current_user.id
...
def delete_one_resume(resume_id: int, db: Annotated[Session, Depends(get_db)], current_user:Annotated[User, Depends(get_current_user)]):
    user_id = current_user.id
...
@router.post('/{resume_id}/improve', response_model=ResumeInDB)
def improve_resume_by_ai(resume_id:int, resume_input:ResumeInput, db: Annotated[Session, Depends(get_db)], current_user:Annotated[User, Depends(get_current_user)]):
    user_id = current_user.id
    query = select(Resume).where(Resume.id == resume_id, Resume.user_id == user_id)
